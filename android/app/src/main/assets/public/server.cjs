...
app.post("/api/patient/chat", async (req, res) => {
  try {
    ...
    if (!client) {
      ...
      const userMessageCount = messages ? messages.filter((m) => m.sender === "user").length : 0;
      ...
      config: {
        ...
        responseSchema: {
          ...
          properties: {
            ...
            overallFeedback: { type: import_genai.Type.STRING },
            historyRating: { type: import_genai.Type.INTEGER },
            examRating: { type: import_genai.Type.INTEGER },
            diagnosticRating: { type: import_genai.Type.INTEGER },
            managementRating: { type: import_genai.Type.INTEGER },
...
Synthesize evidence-based medicine beautifully in Markdown. Include high-risk warning flags if relevant or outline stepwise emergency/elective management plans.`;
    const chatContents = (chatHistory || []).map((msg) => ({
